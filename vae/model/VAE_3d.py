...
def create_3d_VAE_model(input_shape=(4, 160, 192, 128), num_filters_enc=None, num_filters_dec=None, size_vector_latent=256):
    """
    -------------------------------------------
...
Returns
    ...
    if num_filters_enc is None:
        num_filters_enc = (32, 64, 128, 256)
    
    if num_filters_dec is None:
        num_filters_dec = (256, 128, 64, 32)
...
## The Initial Block
    ...
    x1 = green_block(x, num_filters_enc[0], name='x1')
    x = Conv3D(
        filters=num_filters_enc[0],
        kernel_size=(3, 3, 3),
        strides=2,
...
## VAE (Variational Auto Encoder) Part
    ...
    # (10x12x8 neural)
    dec_size_ratio = int(2 ** len(num_filters_enc)) 
    x = Dense((c//4) * (H//dec_size_ratio) * (W//dec_size_ratio) * (D//dec_size_ratio))(x)
    x = Activation('relu')(x)
    # (1x10x12x8)
    x = Reshape(((c//4), (H//dec_size_ratio), (W//dec_size_ratio), (D//dec_size_ratio)))(x)
    x = Conv3D(
        filters=num_filters_dec[-1],
        kernel_size=(1, 1, 1),
        strides=1,
        data_format='channels_first',
    ...
    x = green_block(x, num_filters_dec[-2], name='Dec_VAE_128')

    ### Green Block x1 (output filters=64)
    x = Conv3D(
        filters=num_filters_dec[-3],
        kernel_size=(1, 1, 1),
        strides=1,
        data_format='channels_first',
    ...
    x = green_block(x, num_filters_dec[-3], name='Dec_VAE_64')

    ### Green Block x1 (output filters=32)
    x = Conv3D(
        filters=num_filters_dec[-4],
        kernel_size=(1, 1, 1),
        strides=1,
        data_format='channels_first',
    ...
    x = green_block(x, num_filters_dec[-4], name='Dec_VAE_32')
